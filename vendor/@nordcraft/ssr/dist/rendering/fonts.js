...
export const getFontCssUrl = ({ fonts, baseForAbsoluteUrls, basePath = '/.toddle/fonts/stylesheet/css2', }) => {
    ...
    for (const font of fonts) {
        ...
        // }
        const encodeStaticRange = (range, index) => range
            .map((v) => `${typeof index === 'number' ? `${index},` : ''}${v.weight}`)
            .join(';');
        ...
        const wght = [standardRange, italicRange]
            .map((range, index) => encodeStaticRange(range, hasItalicVariants ? index : undefined))
    ...
    try {
        const url = typeof baseForAbsoluteUrls === 'string'
