 ...
 */
export function setDropHighlight(element, targetContainer, color) {
    highlight?.remove();
    ...
    highlight.style.setProperty('--drop-area-left', `${element.offsetLeft + left}px`);
    ...
    highlight.style.setProperty('--drop-area-border-radius', window.getComputedStyle(element).borderRadius);
    highlight.style.setProperty('--dashed-line-color', `color-mix(in srgb, #${color} 33%, transparent)`);
    document.body.append(highlight);
}
/**
 * Visual representation of where a dragged node will be dropped outside of its own container.
 */
export function setExternalDropHighlight({ layout, center, length, color, projectionPoint, }) {
    highlight?.remove();
    highlight = document.createElement('div');
    ...
    const gradient = `radial-gradient(circle at ${projectionPoint * 100}% ${projectionPoint * 100}%, #${color} 0%, #${color}55 max(100%, 75px))`;
...
//# sourceMappingURL=dropHighlight.js.map